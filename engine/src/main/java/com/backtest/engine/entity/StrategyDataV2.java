...
public class StrategyDataV2 {
	...
	/**
	 * List of stateful safety-net policies (Stage 3a contract). The raw DTOs from
	 * the request. Engine still references this for back-compat / introspection;
	 * runtime dispatch uses {@link #safetyPolicies}.
	 */
	private java.util.List<SafetyNetItemDto> safetyNets;

	/**
	 * Initialised SafetyNetPolicy instances for this regime (Stage 3b). Built by
	 * BacktestContext via SafetyNetRegistry from {@link #safetyNets}. Empty list
	 * means no active policies — strategy trades freely. The day-loop iterates this
	 * list each day to gather freeze/resume decisions.
	 */
	...
	// Hold Blackout — days a stock is frozen from re-entry after it exits.
	// 0 disables. holdBlackoutUnit: "calendar" (default when null) or "trading".
	private int holdBlackoutDays;
	...
	/**
	 * Recalculated once per year (first Jan trading day). Starts at 0 (pass all)
	 * until first recalculation fires. Set via setVolThreshold() in
	 * BacktestServiceImplV2.
	 */
	@Builder.Default
	private volatile float volThreshold = 0f;

	/**
	 * Recalculated once per year (first Jan trading day). Starts at 0 (pass all)
	 * until first recalculation fires.
	 */
	@Builder.Default
	private volatile float turnoverThreshold = 0f;
